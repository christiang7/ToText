...
#echo $ofile
extens=${ofile##*.}
mtype=$(file -b --mime-type "$ofile" | cut -s -d "/" -f 1)
stype=$(file -b --mime-type "$ofile" | cut -s -d "/" -f 2)
Filename=${ofile%.*}
#folder=$(pwd)
...
then
	...
	then
		...
		then
			if [[ webp == $stype ]]
			then
				magick "$folder"/"$Filename".webp "$folder"/"$Filename"-.jpg
				avifenc "$folder"/"$Filename"-.jpg "$folder"/"$Filename".avif > ~/.config/tt/log
				rm "$folder"/"$Filename"-.jpg
			else
				avifenc "$folder"/"$File" "$folder"/"$Filename".avif > ~/.config/tt/log
			fi
	...
	#elif [[ mp4 == $extens || mov == $extens || mkv == $extens || flv = $extens || ogv = $extens || MOV == $extens || MP4 == $extens ]]
	elif [[ video == $mtype ]]
	then
	...
	#then
	#	echo "tue nichts"
	elif [[ $extens == py || $extens == jl  || $extens == css || $extens == js || $extens == html || $extens == c || $extens == cpp || $extens == f || $extens == f90 || $extens == for || $extens == php || $extens == r || $extens == sh || $extens == yml || $extens == desktop ]]
	then
		#file-description "$folder" "$File" "@Document $tags" "$source" "$additiontext" >> "$folder"/"$File".md
