...
### get-langname function - Get language name for file extension

the list of language names are saved in the config file under the variable ``langName``

using ``get-langname`` function with the parameters as following
```bash
    get-langname p1
    p1 - file extension
...
```bash
    markdown-description-program p1 p2
    p1 - cleaned file
    p2 - informations
    output - text
```
