...
then
    ...
    File=$(wget --no-use-server-timestamps --user-agent="Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:88.0)" --no-check-certificate "$url" 2>&1 | tee /dev/tty | grep Wird | cut -d ' ' -f 3 | sed "s/‘//g" | sed "s/’//g")
    #echo $File
