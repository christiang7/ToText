...
#File=$(cleanName "$1")
tabs="$(wl-paste -n)"
sessionfile="$(wl-paste -n | sed -n '1p' | cut -s -d "~" -f 1)"

session=$(yad --title="Insert websession?" --text="\nSave copied tabs\n" \
	--form --width 500 --separator="~" --item-separator=","  \
	--field="Filename" \
    --field="- Insert Headline":CHK \
	"$(date +"%Y-%m-%d")-$sessionfile" "TRUE")
if [ ! $? -eq 1 ];
then
    sessionhead="$(echo "$session" | cut -s -d "~" -f 1)"
    sessionfile="2»ws-$sessionhead"
    ...
    echo "$tabs" >> "$folder"/"$sessionfile".md
