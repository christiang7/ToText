...
#do 
  ...
  then
	...
	function Wikiprev(){
		...
		echo "===== $File =====" >> wiki" >> "$folder"/"$File".txt
		echo "Wiki-Format: zim 0.6" >> "$folder"/"$File".txt
	}

	function Timestamps(){
		echo "Text creation time: $(date +"[[Zettelkasten:%Y:%m:%d]]")" >> "$folder"/"$File".txt
		echo "Modification time: $(date +"[[Zettelkasten:%Y:%m:%d]]" -r "$folder"/"$g")" >> "$folder"/"$File".txt
	}
	if [ -e "$folder"/"$File".txt ]; 
	then
		echo no
	else
	if [[ pdf == $extens ]]
...
then
	Wikiprev
	echo "[*] @EBOOK $3 **[[../$f]] $2**" >> "$folder"/"$File".txt
	Timestamps
	echo -e "[[../]]\n$2\n" >> "$folder"/"$File".txt
	einfo "$folder"/"$g" >> "$folder"/"$File".txt
	#Opentxt
elif [[ eml == $extens ]]
then
	Wikiprev
	echo "[*] @EMAIL $3 **[[../$f]] $2**" >> "$folder"/"$File".txt
	Timestamps
	echo -e "[[../]]\n$2\n" >> "$folder"/"$File".txt
	cat "$folder"/"$g" >> "$folder"/"$File".txt
	#Opentxt
elif [[ maff == $extens ]]
then
	Wikiprev
	folder2=$(unzip -Z -1 "$g" '*/')
	unzip "$f"
	echo "[*] @WEB $3 **[[../$f]] $2 $(cat $folder/$folder2'index.dat' | grep source | cut -f 2)**" >> "$folder"/"$File".txt
	Timestamps
	echo -e "[[../]]\n$2\n" >> "$folder"/"$File".txt
	cat "$folder"/$folder'index.rdf' >> "$folder"/"$File".txt 
...
then
	Wikiprev
	echo "[*] @ORDNER $3 **[[../$g]] $2**" >> "$folder"/"$File".txt
	Timestamps
	echo -e "[[../]]\n$2\n" >> "$folder"/"$File".txt
	#Opentxt
else 
